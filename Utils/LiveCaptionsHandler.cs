...
namespace WindowsLiveCaptionsReader.Utils
{
    public static class LiveCaptionsHandler
    {
        ...
        public static AutomationElement LaunchLiveCaptions()
        {
            ...
            for (int attemptCount = 0;
                 ...
                 attemptCount++)
            {
                if (attemptCount > 10) // Reduced from 10000 for faster failure / dev
                {
                    // Retry with sleep to avoid busy wait
                    Thread.Sleep(500);
                }

                if (process.HasExited)
                {
                     // Try restarting if it exited immediately (some Win versions do this)
                ...
                if (attemptCount > 60) // 30 seconds approx
